...
export const ConfirmEmailPop = ({email , closePopup} : props) => {
    ...
    try {
        const response = await fetch('http://localhost:5000/resend-email', {
            method : 'POST',
    ...
    return (
        <div className="fixed inset-0 w-screen h-screen ">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-5 bg-zinc-800  p-8 shadow-xl  rounded-md">
                ...
                <span className="text-center text-sm text-gray-300">Please click the link in the email we sent to <strong>{email}</strong> to activate your account.</span>
                <button
                    ...
                    >{buttonDisabled ? `Resend email in  ${timeLeft}` : "Resend email"}</button>
            </div>
        </div>
    )
} 
